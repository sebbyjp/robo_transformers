...
class RTDiffusion(Agent):
    ...
    def __init__(self,
                 saved_model_path: str = '/simply_ws/src/RT-X/checkpoints/rtx1_cft_step3500.pt', num_additional_future_actions=4, reduction='mean', *args, **kwargs) -> None:
        ...
        '''
        self.model: RTX1 = RTX1( rt1_config = RT1Config(use_attn_conditioner=False, causal_attention=False),vit_config=FilmViTConfig(pretrained=True))
        if saved_model_path:
    ...
    def act(self,
        ...
        if logging.level_debug():
            for i in range(6):
                Image.fromarray(np.array(images[0,i,:,:,:].permute(1,2,0)*255, dtype=np.uint8)).save('rt1_image_{}.png'.format(i))
     
        if len(self.output_buffer) == 0:
            with torch.no_grad():
                actions_out = rearrange(self.model.run(images, [instruction], cond_scale=1.0), '1 f a bins -> f a bins')[:self.num_predicted_actions,:,:]
