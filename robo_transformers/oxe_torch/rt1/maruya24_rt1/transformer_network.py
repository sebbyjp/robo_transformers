...
class TransformerNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.loss_weight = torch.ones(vocab_size, device=self.device)
    ...
    def _generate_masks(self):
        ...
        self._default_attention_mask = torch.tril(
            torch.ones((self._all_num_tokens, self._all_num_tokens), dtype=torch.uint8, device=self.device))

        action_mask = torch.tensor(
            np.ndarray(shape=(self._all_num_tokens, self._all_num_tokens), dtype=int), device=self.device
        )
    ...
    def forward(
        ...
        if outer_rank == 1:  # This is an inference call
            ...
            action_t = torch.minimum(
                seq_idx, torch.tensor(self._time_sequence_length - 1, device=self.device)
            )
            ...
            network_state["seq_idx"] = torch.minimum(
                seq_idx + 1, torch.tensor(self._time_sequence_length, device=self.device)
            )[None]
            ...
            predicted_action_index = torch.tensor(self._action_tokens_mask, device=self.device) - 1
            ...
            self._aux_info.update(
                {
                    ...
                    "actor_loss_mask": torch.ones((b), dtype=torch.float32, device=self.device),
                }
            )
    ...
    def _assemble_input_token_sequence(
        ...
        action_tokens = torch.zeros_like(
            context_image_tokens, device=self.device
        ...
        input_token_sequence = torch.concat(
            (context_image_tokens.to(self.device), action_tokens), dim=2
        )
        if self.using_proprioception:
        ...
        if outer_rank == 1:  # This is an inference call
            ...
            if (
                ...
                action_tokens = torch.zeros(
                    shape=[b, t, self._tokens_per_action], dtype=torch.int32, device=self.device
                )
            else:
